# Sudoku — 解き方を教える数独

「答えを教える数独」ではなく、**「数独の解き方を教える数独」**。

ヒントを押しても答えは出ません。どの定石を使い、なぜその数字になるのかを、
段階的に・視覚的に説明します。AIは使わず、決定論的なルールベースで推論します。

## 起動

```bash
npm install
npm run dev
```

## スクリプト

| コマンド | 内容 |
|---|---|
| `npm run dev` | 開発サーバー |
| `npm run build` | 本番ビルド |
| `npm run check` | 型チェック＋テスト |
| `npm run typecheck` | 型チェックのみ |
| `npm test` | テストのみ |
| `npm run gen:puzzles` | 事前生成の問題プールを作り直す |
| `npm run gen:test-puzzles` | テクニック別のテスト用固定問題を作り直す |

## 操作

### 入力方式

2通りから選べます。盤面の上のトグルで切り替えます。

| 方式 | 操作 |
|---|---|
| マス優先 | マスを選んでから数字を押す |
| 数字優先 | 数字（または消しゴム）を選んでから、マスをタップする |

数字優先では選んだ数字が保持されるので、同じ数字を続けて何か所にも置けます。
どちらの方式でも「メモ」に切り替えれば候補メモとして入ります。

### 入力のルール

- 答え合わせありです。不正解の数字は赤く表示され、間違いの音が鳴ります
- 正解を入れたマスは確定し、それ以上は書き換えられません（Undo は可能）
- 同じ行・列・ブロックにすでにある数字はメモできません
- ブロック・行・列を正解で埋め切ると、音楽と光の演出が入ります
- 数字優先では、盤面の数字をタップするとその数字を選べます。
  9個とも置き終えた数字は自動で選択が外れます

### 候補メモ

| ボタン | 内容 |
|---|---|
| 候補表示 | 盤面から計算した候補を薄く表示する（書き込みはしない） |
| メモ一括 | 計算した候補を、すべての空きマスにメモとして書き込む |
| メモ消去 | 書き込んだメモをすべて消す |

いずれも Undo で戻せます。

### キーボード（PC）

矢印キーで移動、1〜9 で入力（数字優先では数字の選択）、Enter で確定、
Backspace で消去、Space でメモ切り替え、Esc で数字の選択解除、Ctrl+Z / Ctrl+Shift+Z で戻す・進む。

## 効果音

数字を置いたときの「ポチッ」をはじめ、操作ごとに効果音が鳴ります。
音源ファイルは持たず、Web Audio でその場で合成しています。

- 読み込み待ちが無いので、タップと音がずれない
- オフラインでも鳴る
- 数値を変えるだけで鳴りを調整できる

対応端末では、数字を置いたときにごく短い振動も加わります。
ヘッダーの 🔊 でオン・オフでき、設定は保存されます。

**答え合わせありです。** 不正解の数字を置くと、その場で間違いの音が鳴り、数字と背景が赤くなります。
ブロック・行・列を正解で埋め切るとごほうびの音楽が鳴り、2つ以上が同時に揃うとさらに豪華になります。
書けないメモを数字優先でタップしたときは、マスが揺れて軽い音で知らせます。

## 記録

クリアするとプレイ記録が残ります（ブラウザの localStorage のみ。サーバーには送りません）。

記録画面の主役は「何問解いたか」ではなく **どの定石が苦手か** です。

> その定石が必要だった問題のうち、ヒントに頼った割合

を定石ごとに並べるので、伸ばすべき定石が一目で分かります。
あわせて難易度別の最速・平均タイム、直近30日のプレイ数、最近の履歴を表示します。

## 今日の数独

日付から決定的に問題を選ぶので、サーバーが無くても
同じ日なら同じ問題になり、端末を変えても同じ問題が出ます。
曜日で難易度が変わります（月火はかんたん、水木はふつう、金土はむずかしい、日はエキスパート）。
連続日数も数えます。

## 構成

```text
src/
├── app/                      画面（Home / Play / Stats）
├── components/               盤面・入力・ヒントUI
├── features/audio/           効果音の合成と、鳴らす音の判断
├── features/theme/           ライト / ダークの切り替え
├── features/sudoku/
│   ├── stats/                プレイ記録・集計・デイリー数独
│   ├── board/                盤面データとユーティリティ
│   ├── candidates/           候補計算エンジン
│   ├── solver/               解答・一意解判定（「解けるか」だけを担当）
│   ├── generator/            問題生成・難易度判定・問題供給
│   ├── hints/                ヒントエンジン（「次の一手の導き方」を担当）
│   └── game/                 ゲーム状態・Undo/Redo・保存
└── data/
    ├── puzzles/              事前生成の問題プール（各難易度40問）
    └── test-puzzles/         テクニック別のテスト用固定問題
```

Solver と Hint Engine は役割を分けています。
Solver は「解けるか・解答は何か」だけを担当し、Hint Engine は
「今の盤面からどの論理テクニックで次の一手が導けるか」を担当します。

## ヒントエンジン

やさしい順に検索し、最初に見つかった論理的な手を提示します。

| 順 | 定石 | Lv | 内容 |
|---|---|---|---|
| 1 | Naked Single | 1 | 候補が1つしか残っていないマス |
| 2 | Hidden Single | 1 | その数字を置ける場所がユニット内に1つだけ |
| 3 | Locked Candidates | 1 | ブロックと行・列の重なりで候補を絞る（Pointing / Claiming） |
| 4 | Naked Pair | 1 | 2マスが同じ2候補だけを持つ |
| 5 | Hidden Pair | 2 | 2つの数字が2マスにしか入らない |
| 6 | Naked Triple | 2 | 3マスが3種類の候補を共有 |
| 7 | Hidden Triple | 2 | 3つの数字が3マスにしか入らない |
| 8 | X-Wing | 3 | 2行2列の長方形の関係 |
| 9 | XY-Wing | 3 | 2候補マス3つの連鎖 |
| 10 | XYZ-Wing | 4 | 3候補のマスを軸にした連鎖 |
| 11 | Swordfish | 4 | 3行3列の対応 |
| 12 | Jellyfish | 4 | 4行4列の対応 |

X-Wing・Swordfish・Jellyfish は同じ「フィッシュ」構造（N行がN列に収まる）なので、
`hints/fish.ts` に共通化し、サイズ違いとして実装しています。

各ヒントは「答え」ではなく推論情報を返します。

```ts
type Hint = {
  technique: TechniqueId
  targetCell?: string        // "R4C7"
  value?: number
  relatedCells: string[]
  explanation: string        // 理由の文章
  eliminations: { cell: string; value: number }[]
  units: Unit[]              // 強調する行・列・ブロック
  candidateMarks: CandidateMark[]
  steps: [HintStep, HintStep, HintStep]  // 段階的ヒント
}
```

### 段階的ヒント

| STEP | 内容 | 画面 |
|---|---|---|
| 1 | 考え方 | 関係するユニットだけを強調 |
| 2 | 対象を提示 | 対象セル・根拠セルを強調 |
| 3 | 答えと理論 | 定石名・理由・消える候補を表示 |

定石名は STEP 3 まで伏せています。先に名前が見えると考える余地がなくなるためです。

## 難易度

初期数字の数ではなく、**解くのにどの解法が必要だったか**で判定します。

| 難易度 | 最も難しい解法 |
|---|---|
| かんたん | Naked Single / Hidden Single |
| ふつう | Locked Candidates / Naked Pair |
| むずかしい | Hidden Pair / Triple系 |
| エキスパート | X-Wing / XY-Wing |

生成した問題は必ず「一意解を持つ」かつ「ヒントエンジンだけで最後まで解ける」ことを確認しています。
あてずっぽうでしか解けない問題は採用しません。

## 問題の供給

事前生成プール（各難易度40問）から即座に出題しつつ、裏では Web Worker が
新しい問題を生成して在庫を補充します。開始を待たされず、同じ問題も続きません。

## テスト

ヒントエンジンを重点的にテストしています。
テクニックごとに「そのテクニックが最初に発見される盤面」を固定問題として保存し、

```text
盤面 → Hint Engine → 期待する Technique / Cell / 候補消去
```

を検証します。固定問題は `npm run gen:test-puzzles` で作り直せます。

## 未実装（今後）

- 解法ごとの練習モード・解法チュートリアル
- PWA化・オフライン対応
- Simple Coloring などの連鎖系の解法
