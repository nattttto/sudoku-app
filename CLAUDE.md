...
- **正解を入れたマスは確定し、書き換えも消去もできない**（`isSettled`）。
  ...
  ※これによりプレイヤーは「書き換えられない＝正解」と分かる。
  正誤を隠す設計（間違いの音を重複に限定している理由）とは緊張関係にあるが、
  ユーザーの要望で確定を優先している。
- **同じ行・列・ブロックにある数字はメモできない**（`canAddNote`）。
  すでに書いてあるメモを外す操作はいつでも許す（古い保存データ対策）。
  マス優先のメモ入力では、書けない数字のボタンを押せなくして理由を1行で出す。

## ブロック完成の演出

`GameState.celebration` に「どのブロックが・どのマスで完成したか」を持つ。
reducer を純粋なまま、画面（`BlockCelebration` と `.cell-celebrate`）と効果音（`block`）に伝えるため。

- 正解で3×3を埋め切ったときだけ立つ。最後の一手でクリアした場合はクリアの演出を優先して立てない
